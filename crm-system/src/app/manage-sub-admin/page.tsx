        ...
        <div className="px-4 sm:px-5 pt-4 pb-3 border-b border-crm-border">
          <div className="relative max-w-xs">
            ...
            <input
              ...
              className="w-full h-8 pl-8 pr-3 rounded-lg bg-crm-bg/50 border border-crm-border text-[0.8rem] text-crm-text placeholder:text-crm-text-muted/50 focus:outline-none focus:ring-2 focus:ring-crm-primary/20 focus:border-crm-primary transition-all"
            />
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-crm-border">
                <th className="pl-5 pr-3 py-3.5 text-left text-[0.8rem] font-bold text-crm-text w-[80px]">Id</th>
                <th className="px-5 py-3.5 text-left text-[0.8rem] font-bold text-crm-text">Name</th>
                <th className="px-5 py-3.5 text-left text-[0.8rem] font-bold text-crm-text w-[160px]">Password</th>
                <th className="px-5 py-3.5 text-left text-[0.8rem] font-bold text-crm-text">Email</th>
                <th className="px-5 py-3.5 text-center text-[0.8rem] font-bold text-crm-text w-[120px]">Action</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-crm-border/40 transition-colors hover:bg-crm-primary-muted/20 ${
                      idx % 2 === 1 ? "bg-crm-bg/30" : "bg-crm-card"
                    }`}
                  >
                    <td className="pl-5 pr-3 py-3">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-crm-primary-muted text-crm-primary text-[0.72rem] font-bold tabular-nums">
                        {admin.csvId}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-crm-primary-muted flex items-center justify-center text-crm-primary text-[0.7rem] font-bold shrink-0">
                          {admin.name.slice(0, 2).toUpperCase()}
                        </div>
                        <span className="text-[0.84rem] font-semibold text-crm-text">{admin.name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[0.84rem] text-crm-text-muted font-mono tabular-nums">
                          {pwVisible ? admin.password : "••••••••"}
                        </span>
                        <button
                          onClick={() => togglePasswordVisibility(admin.id)}
                    ...
                    <td className="px-5 py-3">
                      <span className="text-[0.84rem] text-crm-text">{admin.email}</span>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {deleteConfirm === admin.id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() => setDeleteConfirm(null)}
                              className="px-2 py-1 rounded-md bg-crm-bg text-crm-text-muted text-[0.65rem] font-bold hover:bg-crm-border/20 transition-colors"
                            >
                              No
                            </button>
                          </div>
                          ...
                          <button
                            onClick={() => setDeleteConfirm(admin.id)}
                            className="p-1.5 rounded-lg text-crm-border hover:text-red-500 hover:bg-red-50 transition-colors"
                            title="Delete"
                          >
          ...
          <div className="relative bg-crm-card rounded-2xl shadow-2xl w-full max-w-md mx-3 sm:mx-4 animate-[fadeIn_150ms_ease-out] max-h-[92vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 border-b border-crm-border/50 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-crm-primary-muted flex items-center justify-center">
                  {editingAdmin
                    ? <Pencil className="w-4.5 h-4.5 text-crm-primary" strokeWidth={1.8} />
                    : <UserPlus className="w-4.5 h-4.5 text-crm-primary" strokeWidth={1.8} />
                  }
                </div>
                <h3 className="text-[0.95rem] font-bold text-crm-text">
            ...
            <div className="px-4 sm:px-6 py-4 sm:py-5 overflow-y-auto">
              <div className="rounded-xl border border-crm-border p-4 space-y-4">
                ...
                {editingAdmin && (
                  <div>
                    <label className="block text-[0.7rem] font-semibold uppercase tracking-widest text-crm-text-muted mb-1.5">ID</label>
                    <div className="h-10 px-3 rounded-xl bg-crm-bg/50 border border-crm-border flex items-center">
                      <span className="text-[0.84rem] font-bold text-crm-text-muted tabular-nums">{editingAdmin.csvId}</span>
                    </div>
                  </div>
                )}

                <div>
                  <label className="block text-[0.7rem] font-semibold uppercase tracking-widest text-crm-text-muted mb-1.5">Name</label>
                  <input
                    type="text"
                    value={formName}
                    onChange={(e) => setFormName(e.target.value)}
                    placeholder="Enter name"
                    className="w-full h-10 px-3 rounded-xl bg-crm-bg/50 border border-crm-border text-[0.84rem] text-crm-text placeholder:text-crm-text-muted/50 focus:outline-none focus:ring-2 focus:ring-crm-primary/20 focus:border-crm-primary transition-all"
                  />
                </div>

                <div>
                  <label className="block text-[0.7rem] font-semibold uppercase tracking-widest text-crm-text-muted mb-1.5">Password</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-10 px-3 pr-10 rounded-xl bg-crm-bg/50 border border-crm-border text-[0.84rem] text-crm-text placeholder:text-crm-text-muted/50 focus:outline-none focus:ring-2 focus:ring-crm-primary/20 focus:border-crm-primary transition-all"
                    />
                    <button
                      type="button"
                ...
                <div>
                  <label className="block text-[0.7rem] font-semibold uppercase tracking-widest text-crm-text-muted mb-1.5">Email</label>
                  <input
                    type="email"
                    value={formEmail}
            ...
            <div className="flex items-center justify-end gap-2 px-4 sm:px-6 pb-4 sm:pb-5 pt-2 shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-2 h-11 px-5 rounded-xl bg-crm-primary text-white text-[0.82rem] font-medium hover:bg-[#4845a2] active:bg-crm-sidebar transition-colors shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {saving ? (
