  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <select className="h-9 px-3 pr-8 rounded-xl bg-white border border-slate-100 text-[0.82rem] text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-200 appearance-none cursor-pointer">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map(({ label, value, prev, icon: Icon }) => (
          <div key={label} className="bg-white rounded-2xl p-5 card-shadow">
            ...
            <p className="text-[1.6rem] font-semibold tracking-tight text-slate-900 leading-none mb-1">
              {value}
            </p>
            <p className="text-[0.76rem] text-slate-400">
              {label} <span className="text-slate-300">vs {prev}</span>
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 card-shadow">
          <h3 className="text-[0.95rem] font-semibold text-slate-800 mb-6">
            Deals Trend
          </h3>
          <div className="flex items-end gap-3 h-48">
            {monthlyData.map((d) => (
              <div key={d.month} className="flex-1 flex flex-col items-center gap-2">
                ...
                <div
                  className="w-full rounded-t-lg bg-blue-400/80 hover:bg-blue-500 transition-colors cursor-pointer"
          ...
          <div className="divide-y divide-slate-50">
            {topPerformers.map((person, i) => (
              <div key={i} className="flex items-center gap-3 px-6 py-3.5">
                <span className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-[0.65rem] font-bold text-blue-700">
                  {i + 1}
                </span>
                ...
                <span className="text-[0.82rem] font-semibold text-blue-600">
                  {person.revenue}
                </span>
              </div>
            ))}
