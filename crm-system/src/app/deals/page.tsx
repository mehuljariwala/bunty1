    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-[0.85rem] text-slate-400">
          Pipeline value: <span className="font-semibold text-slate-700">$156,100</span>
        </p>
        ...
        {stages.map((stage) => (
          <div key={stage.name} className="space-y-3">
            <div className="flex items-center gap-2.5 px-1">
              ...
              <h3 className="text-[0.82rem] font-semibold text-slate-700">
                {stage.name}
              </h3>
              <span className="ml-auto text-[0.72rem] font-medium text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full">
                {stage.deals.length}
              </span>
            </div>
              ...
              {stage.deals.map((deal, i) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2.5">
                    <p className="text-[0.84rem] font-medium text-slate-800 leading-snug pr-2">
                      {deal.title}
                    </p>
                    <ArrowUpRight className="w-3.5 h-3.5 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5" />
                  </div>
                  <p className="text-[1.1rem] font-semibold text-slate-900 mb-2">
                    {deal.value}
                  </p>
                  <div className="flex items-center justify-between">
